...
这里只有客户端。服务端要做的事：验签、k-匿名过滤、按公钥封禁、
`ephemeralId → 公钥` 的映射。契约见技术架构第 8 节，
但注意上面四条已经修正了其中两处。
...
`server/` 是个零依赖的最小实现（`node:http` + `node:crypto`），能在任何装了
Node 的地方跑。起它：
...
`-k 1` 是单机自测用的。默认门槛 5，单机永远凑不够 —— relevant() 返回空是
**正确**行为。调它就是在关掉一条隐私红线。

配合客户端：`cordis.yml` 里把 endpoint 填成 `http://127.0.0.1:4000`。
...
| k-匿名按**发布者数**算 | 按卡片数算的话，一个人发 N 张就能让自己的话题变得「可推荐」，信息回流 |
| 不回传 publisher | 返回发布者公钥等于公开谁发了什么 |

三条都做了变异测试：分别去掉后各触发 1 个失败。

### 部署在哪 —— 还没定

目前只在本机跑过。选主机时要想清楚的是**数据归属**：服务端持有
`公钥 → 卡片` 的映射，谁拿到这份数据，谁就能把一个人的所有发言串起来。
这是整个信任模型的锚点。

存储现在是纯内存（`server/src/store.ts`），重启即失。换主机时存储方案会跟着变
（SQLite / Postgres / KV），所以刻意没提前选一个绑死 —— 那一层接口很窄，
换实现改一个文件。

### 已知缺口

- **签名可重放**：没有 nonce 和时间戳。实际影响有限（重放 publish 产生重复卡片，
  重放 delete 幂等），但接真实流量前应加 `x-social-timestamp` 并拒绝过期请求。
- **k-匿名是逐查询过滤，不是真正的保证**：攻击者可以变换查询向量逐步逼近。
  真实部署还需要查询频率限制。
- 无速率限制、无内容审核、无 HTTPS（靠反向代理提供）。
