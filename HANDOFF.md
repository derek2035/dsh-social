...
├── server/         最小服务端（零依赖，node:http）。可单独部署
├── packages/
│   ├── cloud/      [Provider]    联网。★唯一发网络请求的包
...
## cloud provider：契约里的四个洞

写的时候发现技术架构第 8 节那份契约有四处在 local 下不存在、一联网就冒出来的问题。
四处都已按下面的方式落地，改之前请先读这一节。

**① `DELETE /v1/cards/{cardId}` 原本没有鉴权。**
契约只写了 `resp: 204`。照那样实现，任何人拿到 cardId 就能删别人的卡片 ——
而 `relevant()` 返回的正是 cardId。现在发布和撤回都带 Ed25519 签名。

**② 身份模型：设备级匿名密钥，不是账号。**
首次使用本地生成一对 Ed25519 密钥，公钥当身份（契约里的 `ephemeralId`），
私钥存 `ctx.credentials`，永不离开本机。两个明确的代价写在
`packages/cloud/src/identity.ts` 文件头，**应该写进产品的隐私说明**：
换设备等于换身份（旧卡撤不回来）；删掉密钥可以绕过封禁。

**③ `relevant()` 会泄露从未发布过的话题。** ← 最要紧的一条
`publish` 上传的向量对应已公开的文字，无额外泄露。但 `relevant()` 的向量
来自调用方给的文本 —— 拿用户当前正在聊的内容去查，等于持续上传他从没同意
公开的话题。`topicVector` 是 64 维哈希词袋，不可直接求逆，但对短文本做字典
攻击能还原相当一部分。**别把「不可逆」当成「安全」。**

约定：**只在用户按下发布之后调用，且只用那张已公开卡片的向量。**
「边聊边推荐」要做也必须是用户显式打开的开关，不能是默认行为。
目前没有任何代码调用 `relevant()`。

**④ 网络失败时转化率会系统性偏低。**
原来 `/social-publish` 只在 `publish()` 成功后才记 DecisionRecord。
联网后请求可能失败，而**用户的决定确实发生了** —— 那正是要测的东西。
现在改成：失败也记，只是不带 cardId。
所以 `decision: 'published'` 且无 `cardId` 的含义是「决定了但没送达」。

另外 `retract` 的三态必须分清：204 → `true`，404 → `false`，
**超时/5xx → 抛错**。把超时当成 `false` 是在撒谎，和之前修掉的那个
「已撤回」bug 是同一类错误。

### 怎么切过去

`cordis.yml` 里注释掉 `social-local` 那段，取消注释 `social-cloud` 那段，填 endpoint。
consumer 一行都不用改。bundle 版改 `bundle/cordis.patch.yml`，
`name` 用 `dsh-social-plugin/cloud`。

### 服务端

`server/` 是零第三方依赖的实现（`node:http` + `node:crypto` + `node:sqlite`）。

```bash
node --experimental-strip-types server/bin.ts \
  --db ~/.dsh-social/server.db --port 4000 -k 1
```

`--db` 省略则用内存库（测试用，重启即失）。`-k 1` 只在单机自测时用 ——
默认门槛 5，单机凑不够，`relevant()` 返回空是**正确**行为，
调它就是在关掉一条隐私红线。

客户端：`cordis.yml` 里 cloud 段的 endpoint 填 `http://127.0.0.1:4000`。

### 五条防线（每条都有测试 + 变异验证）

| 防线 | 挡住什么 |
|---|---|
| 撤回验签**且验所有权** | 只凭 cardId 就能删 = 把删除权发给所有读者，而 relevant() 返回的正是 cardId |
| **入站向量重新归一化** | 见下，这是最隐蔽的一条 |
| 时间戳参与签名 + 签名当 nonce | 重放。缺任一条都不完整：只有时间戳则窗口内可重放，只有 nonce 则表无限增长 |
| 按公钥限流（默认攒 10 次、每 10 秒回 1 次） | 刷屏。身份层拦不住（删掉密钥就换了身份），这才是真正的防滥用手段 |
| k-匿名按**发布者数**算 | 一个人发 N 张同话题的卡就让自己的话题变得「可推荐」，信息原路回流 |

### ★ 入站向量为什么必须重新归一化

`packages/core/src/topic.ts` 里的 `cosine()` **其实是点积** —— 它省掉了
除以模长那一步，前提是两个向量都已 L2 归一化。本地场景成立（向量都是
`topicVector()` 自己算的），但服务端收的是**客户端给的向量**，前提不再成立：

- 查询时传一个放大 1000 倍的向量 → 点积跟着放大 1000 倍 →
  本来 0.126 的弱相关变成 126，轻松越过 0.25 的阈值 →
  话题范围形同虚设，攻击者能把弱相关的卡片全捞出来
- 发布时存一个大模长向量 → 这张卡匹配所有人的所有查询

所以 `server/src/vector.ts` 对入站向量一律：校验维度（必须是 `TOPIC_DIM`）、
校验有限性（NaN / Infinity 会污染后续所有比较）、**服务端自己重新归一化**。

归一化而不是拒绝，是因为善意客户端的浮点误差不该被当成攻击；
而对攻击者来说，归一化之后放大模长这条路就没有收益了。

写这条测试时踩了个坑：第一版用「全 1 向量 × 1000」当攻击载荷，
测试通过但变异测试抓不到 —— 因为 `topicVector` 的符号是 ±1 散列的，
全 1 向量跟任何卡片点积都会大量抵消。换成「弱相关文本的向量 × 1000」
才真正复现了这条攻击。**测试通过不等于测试有效。**

### 存储

SQLite，不是 JSON 文件：卡片会持续增长，而撤回必须真删 ——
每次撤回重写整个文件，几千张卡之后会开始丢写。

表：`cards`（含 publisher，**绝不返回客户端**）、`bans`、`seen_nonces`
（定期清掉时间窗外的，不会无限增长）。

### 部署在哪 —— 还没定

只在本机跑过。选主机时要想清楚的是**数据归属**：服务端持有
`公钥 → 卡片` 的映射，谁拿到这份数据，谁就能把一个人的所有发言串起来。
这是整个信任模型的锚点。

### 还缺的

- **无 HTTPS** —— 靠反向代理提供。裸 HTTP 上公网等于把签名和内容明文广播
- **无内容审核** —— 这是产品决策，不是技术缺口
- **限流是进程内状态** —— 多实例部署要换共享存储，但那时整个部署形态都变了
- **k-匿名是逐查询过滤，不是真正的保证** —— 变换查询向量可以逐步逼近，
  还需要查询频率限制（目前只限了写操作）
...
## 话题内对话（2026-08-16 真机跑通）

别人的发言会作为「上下文注入」行出现在你的会话流里，形态和 DSH 自己的
time-context / skill-catalog 完全一样。

```
/social-join <cardId前缀>       关注话题
/social-say <cardId前缀> <话>   发言
/social-leave <cardId前缀>      取关
```

要让别人的发言真的进会话流，还得把 `social-curator` 的 `injectTopics`
改成 `true`（默认 false）。

### ★ injectTopics 为什么默认关闭

打开它 = 跨过设计文档第 7 节划给阶段 4 的那条线，接受三件事：

1. **花 token** —— 别人的话进 prompt，每轮都发
2. **AI 会接话** —— 它看得见，可能主动评论陌生人的观点（文档说的「打断」）
3. **别人的话落进你本机会话日志**，永久保留（文档列的第一条隐私顾虑）

用 `form: 'snapshot'` 让成本有上界：快照可替换，只带最近 12 条，不累积。

### ★ 每话题化名

`alias = hash(公钥 + 话题id + 服务端盐)`，见 `server/src/alias.ts`。

- 同话题内稳定 → 对话读得通（「路人 afnd」始终是同一个人）
- 跨话题不同 → 串不起来
- 服务端有盐 → 猜到公钥也算不出化名

真机验证过：同一个人在一个话题里的两条发言都是 `afnd`，另一人是 `e48w`，
公钥不出服务端。

**但这是「对用户匿名，对服务端实名」**，服务端仍持有映射。
盐是进程级的，重启会让所有化名变化 —— 真实部署要持久化它。

### 两处早先的错判（别再重复）

1. 我一度断言「别人的消息进不了会话流」。错了 —— 那是把「插入自定义
   **事件类型**」（确实被 `KNOWN_SESSION_EVENT_TYPES` 挡住）和「插入**消息**」
   混为一谈。后者可以：`user/message` + `source {kind:'plugin'}`。
2. 设计文档第 7 节被我读窄过。原文「只**渲染给用户看**，不进模型上下文」——
   反对的是进模型上下文，渲染给用户看恰恰是它预期的。

### 又一次栽在 inject 上

curator 用了 `ctx.social` 但没在 `inject` 里声明，真机报
`cannot get property "social" without inject`。关键是它**只在运行到那行时才炸**，
启动阶段一切正常、日志干净。这是这个项目第二次栽在 inject 上
（第一次是 `{required, optional}` 那个写法）。

**用了哪个 ctx.X 就得 inject 哪个，没有例外。**
